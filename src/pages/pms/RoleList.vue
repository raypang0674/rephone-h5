    ...
    <el-table ref="elTable" :data="tableData" class="table" stripe empty-text="没有匹配到数据" @row-click="clickRow" @selection-change="handleTableSelectionChange">
      ...
      <el-table-column fixed="right" label="操作" min-width="120" align="center">
        <template slot-scope="scope">
          ...
          <el-button @click.native.prevent="editPermissionRel(scope.row, scope.$index)" @click.stop="stopEvent" type="text" size="small">关联权限</el-button>
        </template>
      </el-table-column>
    </el-table>
    <Paging @currentChange="goPage" :total="total" :layout="pageLayout" :pageSize="pageSize" :currentPage="page"></Paging>

    <el-dialog title="编辑角色" :visible.sync="eForm.visible" @closed="formClosed('editForm')" width="500px">
...
export default {
  ...
  methods: {
    ...
    refreshList() {
      this.$AxiosWithToken({
        url: this.PMS_API_HOST + '/api/auth/pms/roleList',
        // method: 'post',
        params: {
    ...
    editRow(row, index) {
      this.$AxiosWithToken({
        url: this.PMS_API_HOST + '/api/auth/pms/role',
        // method: 'post',
        params: {
          // 用data适合后台用@RequestBody方式
          id: row.id
    ...
    doDeleteSelected() {
      ...
      this.$AxiosWithToken({
        url: this.PMS_API_HOST + '/api/auth/pms/roleDelete',
        // method: 'post',
        params: {
          // 用data适合后台用@RequestBody方式
          id: ids
    ...
    formConfirm(formName) {
      this.$refs[formName].validate(state => {
        ...
        this.$AxiosWithToken({
          url: this.PMS_API_HOST + '/api/auth/pms/roleEdit',
          // method: 'post',
          params: {
            // 用data适合后台用@RequestBody方式
            id: this.eForm.id,
    ...
    editPermissionRel(row, index) {
      this.$router.push({
        query: {
          role: row.id,
          // parent: 0,
          title: '角色[' + row.name + ']的权限',
          crumbs: '角色管理,关联权限'
        },
