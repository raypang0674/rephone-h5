    ...
    <el-table ref="elTable" :data="tableData" class="table" stripe empty-text="没有匹配到数据" @row-click="clickRow" @selection-change="handleTableSelectionChange">
      ...
      <el-table-column fixed="right" label="操作" min-width="120" align="center">
        <template slot-scope="scope">
          ...
          <el-button @click.native.prevent="editSubMenu(scope.row, scope.$index)" @click.stop="stopEvent" type="text" size="small">子菜单</el-button>
        </template>
      </el-table-column>
    </el-table>
    <Paging @currentChange="goPage" :total="total" :layout="pageLayout" :pageSize="pageSize" :currentPage="page"></Paging>

    <el-dialog title="编辑菜单" :visible.sync="eForm.visible" @closed="formClosed('editForm')" width="500px">
      <el-form ref="editForm" :model="eForm" :rules="ruleForm" label-width="80px">
        ...
        <el-form-item label="图标" prop="icon">
          <el-input v-model="eForm.icon"></el-input>
        </el-form-item>
...
export default {
  data() {
    ...
    let vldUrl = (rule, value, callback) => {
      if (value) {
        if (!/^[\w\W]{1,50}$/.test(value)) {
    ...
    let vldIcon = (rule, value, callback) => {
      if (value) {
        if (!/^[\w\W]{1,50}$/.test(value)) {
          callback(new Error('长度不超过50'))
        } else {
    ...
    return {
      ...
      ruleForm: {
        ...
        url: [{ validator: vldUrl, trigger: 'blur' }],
        icon: [{ validator: vldIcon, trigger: 'blur' }]
      },
      tableData: [],
      queryCondition: {
        name: ''
  ...
  watch: {
    ...
    $route: function(to) {
      this.application = to.query.app || 0
      this.parentMenu = to.query.parent || 0
  ...
  methods: {
    ...
    refreshList() {
      this.$AxiosWithToken({
        url: this.PMS_API_HOST + '/api/auth/pms/menuList',
        // method: 'post',
        params: {
    ...
    editRow(row, index) {
      this.$AxiosWithToken({
        url: this.PMS_API_HOST + '/api/auth/pms/menu',
        // method: 'post',
        params: {
          // 用data适合后台用@RequestBody方式
          id: row.id
        ...
        if (res && res.code) {
          if (res.code === 200) {
            ...
            this.eForm.url = res.data.url
            this.eForm.icon = res.data.icon
    ...
    doDeleteSelected() {
      ...
      this.$AxiosWithToken({
        url: this.PMS_API_HOST + '/api/auth/pms/menuDelete',
        // method: 'post',
        params: {
          // 用data适合后台用@RequestBody方式
          id: ids
    ...
    formConfirm(formName) {
      this.$refs[formName].validate(state => {
        ...
        this.$AxiosWithToken({
          url: this.PMS_API_HOST + '/api/auth/pms/menuEdit',
          // method: 'post',
          params: {
            // 用data适合后台用@RequestBody方式
            id: this.eForm.id,
  ...
  mounted() {
    this.application = this.$route.query.app || 0
    this.parentMenu = this.$route.query.parent || 0
    this.refreshList()
  },
  components: {
    Paging
