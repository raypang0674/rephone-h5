    ...
    <el-table ref="elTable" :data="tableData" class="table" stripe empty-text="没有匹配到数据" @row-click="clickRow" @selection-change="handleTableSelectionChange">
      ...
      <el-table-column fixed="right" label="操作" min-width="120" align="center">
        <template slot-scope="scope">
          ...
          <el-button @click.native.prevent="editMenuRel(scope.row, scope.$index)" @click.stop="stopEvent" type="text" size="small">关联菜单</el-button>
        </template>
      </el-table-column>
    </el-table>
    <Paging @currentChange="goPage" :total="total" :layout="pageLayout" :pageSize="pageSize" :currentPage="page"></Paging>

    <el-dialog title="编辑权限" :visible.sync="eForm.visible" @closed="formClosed('editForm')" width="500px">
...
export default {
  ...
  methods: {
    ...
    refreshList() {
      this.$AxiosWithToken({
        url: this.PMS_API_HOST + '/api/auth/pms/permissionList',
        // method: 'post',
        params: {
    ...
    editRow(row, index) {
      this.$AxiosWithToken({
        url: this.PMS_API_HOST + '/api/auth/pms/permission',
        // method: 'post',
        params: {
          // 用data适合后台用@RequestBody方式
          id: row.id
    ...
    doDeleteSelected() {
      ...
      this.$AxiosWithToken({
        url: this.PMS_API_HOST + '/api/auth/pms/permissionDelete',
        // method: 'post',
        params: {
          // 用data适合后台用@RequestBody方式
          id: ids
    ...
    formConfirm(formName) {
      this.$refs[formName].validate(state => {
        ...
        this.$AxiosWithToken({
          url: this.PMS_API_HOST + '/api/auth/pms/permissionEdit',
          // method: 'post',
          params: {
            // 用data适合后台用@RequestBody方式
            id: this.eForm.id,
    ...
    editMenuRel(row, index) {
      this.$router.push({
        query: {
          permission: row.id,
          // parent: 0,
          title: '权限[' + row.name + ']的菜单',
          crumbs: '权限管理,关联菜单'
        },
