...
export default {
  ...
  methods: {
    ...
    refreshList() {
      ...
      this.$AxiosWithToken({
        url: this.BIZ_API_HOST + '/api/auth/merchant/callerList',
        // method: 'post',
        params: {
    ...
    editRow(row, index) {
      if (!this.shortCode) {
        this.$message.warning('自身信息读取失败')
        return
      }
      this.$AxiosWithToken({
        url: this.BIZ_API_HOST + '/api/auth/merchant/caller',
        // method: 'post',
        params: {
          // 用data适合后台用@RequestBody方式
          id: row.id
    ...
    doLockCaller(id) {
      this.$AxiosWithToken({
        url: this.BIZ_API_HOST + '/api/auth/merchant/callerDelete',
        // method: 'post',
        params: {
          // 用data适合后台用@RequestBody方式
          id: id
    ...
    doResumeCaller(id) {
      this.$AxiosWithToken({
        url: this.BIZ_API_HOST + '/api/auth/merchant/callerResume',
        // method: 'post',
        params: {
          // 用data适合后台用@RequestBody方式
          id: id
    ...
    doUnBindCallNumber(n) {
      this.$AxiosWithToken({
        url: this.BIZ_API_HOST + '/api/auth/merchant/callerUnBindCallNumber',
        // method: 'post',
        params: {
          // 用data适合后台用@RequestBody方式
          number: n
    ...
    formConfirm(formName) {
      this.$refs[formName].validate(state => {
        ...
        let reqParam
        if (this.eForm.id > 0) {
          editUrl = this.BIZ_API_HOST + '/api/auth/merchant/callerEdit'
          reqParam = {
            id: this.eForm.id,
    ...
    resetPwdFormConfirm(formName) {
      this.$refs[formName].validate(state => {
        ...
        this.$AxiosWithToken({
          url: this.BIZ_API_HOST + '/api/auth/merchant/userResetPassword',
          method: 'post',
          data: {
            id: this.pwdForm.id,
    ...
    reloadNumberA4Select(query) {
      ...
      this.$AxiosWithToken({
        url: this.BIZ_API_HOST + '/api/auth/merchant/callNumberList4Select',
        // method: 'post',
        params: {
          number: query,
          numberType: 1,
    ...
    loadSelf() {
      this.$AxiosWithToken({
        url: this.BIZ_API_HOST + '/api/auth/merchant/self'
        // method: 'post',
        // params: {}
      }).then(res => {
        if (res && res.code) {
          if (res.code === 200) {
            if (res.data.shortCode) {
              this.shortCode = res.data.shortCode + '_'
              return
            }
          }
        }
