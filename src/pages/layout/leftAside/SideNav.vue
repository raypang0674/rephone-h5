  ...
  <div :class="{ 'menu-wrapper': state }">
    ...
    <el-submenu v-else :index="String(index + 1)">
      ...
      <template v-for="(cItem, cIndex) in item.child" :index="String(index + 1 + '-' + cIndex + 1)">
        <el-menu-item v-if="!cItem.child" :index="cItem.path" :key="cItem.id">
          <i v-if="cItem.icon" :class="cItem.icon" :style="{ color: getTextColor }"></i>
          <span slot="title">{{ cItem.title }}</span>
        </el-menu-item>

        <el-submenu v-else :index="String(index + 1 + '-' + cIndex + 1)" :key="cItem.id">
          <i v-if="cItem.icon" :class="cItem.icon" :style="{ color: getTextColor }"></i>
          <span slot="title">{{ cItem.title }}</span>
          <!--递归自己 遍历子..导航-->
          <template v-for="(ccItem, parentIndex) in cItem.child">
            <SideNav :item="ccItem" :index="parentIndex" :key="ccItem.id"></SideNav>
          </template>
        </el-submenu>
      </template>
    </el-submenu>
